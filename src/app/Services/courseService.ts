...
export class CourseService {
    ...
    labelDB: LabelDB[] = [
        ...
        new LabelDB("courseName", "Course Name", Languages.English), new LabelDB("price", "Kurs Fiyat", Languages.Turkish),
        new LabelDB("price", "Course Price", Languages.English),
        new LabelDB("hours", "Kurs Süresi", Languages.Turkish), new LabelDB("hours", "Course Totla Hours", Languages.English)
    ];
    ...
    errorMessage: LabelDB[] = [
        ...
        new LabelDB("timeRequried", "Toplam Süre Alanı Zorunludur!", Languages.Turkish),
        new LabelDB("timeRequried", "Total Hours Field Required!", Languages.English),
        new LabelDB("openCourse", "Açılacak Kurslar", Languages.Turkish),
        new LabelDB("openCourse", "Open Courses", Languages.English),
        new LabelDB("courseDescription", "Herbir açılacak kursun Adı, Fiyatı ve Süresi zorunludur:", Languages.Turkish),
        new LabelDB("courseDescription", "Name, Price and Duration of each course to be opened is mandatory:", Languages.English)
    ];
    ...
    public GetColumnDefData(languageID: Languages = Languages.Turkish) {
        ...
        switch (languageID) {
            ...
            case Languages.English:
                columnDefList = [new ColumnDefs("Course Name", "Name"), new ColumnDefs("Course Price", "Price"), new ColumnDefs("Total Hours", "TotalHours"), new ColumnDefs("Id", "Id")];
                break;
        }
    ...
    public GetErrortext(key: string, languageID: Languages = Languages.Turkish): string {
        if (this.errorMessage.filter(txt => txt.key == key && txt.languageID == languageID).length > 0) {
            return this.errorMessage.filter(txt => txt.key == key && txt.languageID == languageID)[0].value;
        }
    }

    public GetButtonText(key: string, languageID: Languages = Languages.Turkish): string {
        if (this.buttonText.filter(btn => btn.key == key && btn.languageID == languageID).length > 0) {
            return this.buttonText.filter(btn => btn.key == key && btn.languageID == languageID)[0].value;
        }
    }
}